import React from 'react'
import MainLayout from '@/Layouts/MainLayout'
import { Head } from '@inertiajs/react'

function SGCSMRules() {
    return (
        <MainLayout>
            <Head title="SGCSM Rules" />

            <main class="bg-slate-50">
                <div class="bg-gray-100 min-h-screen">

                    <section className="relative overflow-hidden bg-slate-950">
                        <img src="https://images.unsplash.com/photo-1522202176988-66273c2fd55f?q=80&w=1800&auto=format&fit=crop"
                            alt="" className="absolute inset-0 w-full h-full object-cover opacity-35" />
                        <div className="absolute inset-0 bg-gradient-to-r from-slate-950 via-blue-950/85 to-slate-900/25"></div>

                        <div className="relative max-w-7xl mx-auto px-6 py-16 md:py-20">
                            <div className="grid lg:grid-cols-12 gap-10 items-end">
                                <div className="lg:col-span-8 text-white">
                                    <span
                                        className="inline-flex items-center gap-2 bg-white/15 px-4 py-2 rounded-full text-sm font-semibold">
                                        <i className="fa-solid fa-building-columns"></i>
                                        SGCSM | Rules & Regulations
                                    </span>

                                    <h1 className="mt-5 text-4xl md:text-6xl font-black leading-tight uppercase">
                                        Rules & Regulations
                                    </h1>

                                </div>

                            </div>
                        </div>
                    </section>

                    <section class="max-w-7xl mx-auto px-6 py-16">

                        <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">

                            <div class="lg:col-span-2">

                                <div class="bg-white rounded-[35px] shadow-2xl border overflow-hidden">

                                    <div class="bg-gray-900 px-8 py-6">

                                        <h2 class="text-3xl font-bold text-white">
                                            Organization Rules & Policies
                                        </h2>

                                    </div>

                                    <div class="p-8 md:p-10">

                                        <div class="space-y-8">

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-blue-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-blue-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    SGCSM is not responsible for any loss or damage
                                                    incurred from dealing with any third party.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-red-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-red-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    Admission of any candidate may be cancelled if found
                                                    involved in criminal activities.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-green-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-green-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    All institutes will work as authorized study centres
                                                    of S.G.C.S.M / S.G.W.T.I / S.G.Y.S / S.G.A.T.I, New Delhi.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-yellow-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-yellow-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    All candidates of the centre must follow the rules
                                                    laid down by the Center Head.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-purple-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-purple-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    Information provided by users such as IDs and passwords
                                                    will not be shared with any third party.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-pink-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-pink-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    All courses run by SGCSM / SGWTI / SGYS are valid
                                                    for self employment opportunities.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-cyan-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-cyan-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    SGCSM will not receive cash from Centre Directors.
                                                    All charges must be paid through Net Banking (SBI & PNB).
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-indigo-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-indigo-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    Free education facilities are available for
                                                    physically handicapped students.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-orange-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-orange-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    Monthly reports must be submitted in proper format
                                                    to Head Office by Branch Heads.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-gray-200 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-gray-700" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    No amount of any kind will be refunded under any circumstances.
                                                </p>

                                            </div>

                                            <div class="flex items-start gap-5">

                                                <div class="w-12 h-12 rounded-2xl bg-emerald-100 flex items-center justify-center shrink-0">

                                                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-emerald-600" fill="none"
                                                        viewBox="0 0 24 24" stroke="currentColor">

                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />

                                                    </svg>

                                                </div>

                                                <p class="text-gray-700 text-lg leading-relaxed">
                                                    All institutes must work according to the rules
                                                    and regulations of the organization.
                                                </p>

                                            </div>

                                        </div>

                                    </div>

                                </div>

                            </div>

                            <div>

                                <div class="sticky top-10">

                                    <div class="bg-white rounded-[35px] shadow-2xl border overflow-hidden">

                                        <img src="https://images.unsplash.com/photo-1450101499163-c8848c66ca85?q=80&w=1200&auto=format&fit=crop"
                                            alt="Rules and Regulations" class="w-full h-72 object-cover" />

                                        <div class="p-8">

                                            <h3 class="text-3xl font-bold text-gray-900">
                                                Important Notice
                                            </h3>

                                            <p class="text-gray-600 text-lg mt-5 leading-relaxed">
                                                Every student, institute, and authorized study centre
                                                must follow the official rules and policies to maintain
                                                transparency, discipline, and educational standards.
                                            </p>

                                            <div class="mt-8 space-y-4">

                                                <div class="flex items-center gap-4">

                                                    <div class="w-10 h-10 rounded-xl bg-blue-100 flex items-center justify-center">
                                                        ✓
                                                    </div>

                                                    <p class="text-gray-700">
                                                        Fair & Transparent Policies
                                                    </p>

                                                </div>

                                                <div class="flex items-center gap-4">

                                                    <div class="w-10 h-10 rounded-xl bg-green-100 flex items-center justify-center">
                                                        ✓
                                                    </div>

                                                    <p class="text-gray-700">
                                                        Student Safety & Security
                                                    </p>

                                                </div>

                                                <div class="flex items-center gap-4">

                                                    <div class="w-10 h-10 rounded-xl bg-purple-100 flex items-center justify-center">
                                                        ✓
                                                    </div>

                                                    <p class="text-gray-700">
                                                        Quality Educational Standards
                                                    </p>

                                                </div>

                                            </div>



                                        </div>

                                    </div>

                                </div>

                            </div>

                        </div>

                    </section>

                </div>
            </main>
        </MainLayout>
    )
}

export default SGCSMRules